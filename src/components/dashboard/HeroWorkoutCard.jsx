import { Flame, Timer, Zap, GaugeCircle } from "lucide-react"
import BaseCard from "./BaseCard"
import AnimatedProgressBar from "./AnimatedProgressBar"

function HeroWorkoutCard({
	title,
	type,
	durationMinutes,
	difficulty,
	estimatedCalories,
	weeklyCompletionPercent,
	isLoading = false,
}) {
	return (
		<BaseCard className="p-5 sm:p-6">
			{isLoading ? (
				<div className="space-y-4 animate-pulse">
					<div className="h-4 w-36 rounded bg-zinc-700" />
					<div className="h-8 w-52 rounded bg-zinc-700" />
					<div className="grid grid-cols-2 gap-3">
						<div className="h-16 rounded-xl bg-zinc-700" />
						<div className="h-16 rounded-xl bg-zinc-700" />
					</div>
					<div className="h-11 rounded-xl bg-zinc-700" />
				</div>
			) : (
				<>
					<p className="text-xs uppercase tracking-wider text-orange-400">
						Today's Workout
					</p>
					<h2 className="mt-2 text-2xl sm:text-3xl font-bold text-zinc-50">
						{title}
					</h2>
					<p className="mt-1 text-sm text-zinc-400">{type}</p>

					<div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
						<div className="rounded-xl bg-zinc-800/80 p-3">
							<div className="flex items-center gap-2 text-zinc-300 text-xs">
								<Timer className="h-3.5 w-3.5 text-orange-400" /> Duration
							</div>
							<p className="mt-1 text-zinc-100 font-semibold">
								{durationMinutes} min
							</p>
						</div>
						<div className="rounded-xl bg-zinc-800/80 p-3">
							<div className="flex items-center gap-2 text-zinc-300 text-xs">
								<GaugeCircle className="h-3.5 w-3.5 text-orange-400" />
								Difficulty
							</div>
							<p className="mt-1 text-zinc-100 font-semibold">{difficulty}</p>
						</div>
						<div className="rounded-xl bg-zinc-800/80 p-3">
							<div className="flex items-center gap-2 text-zinc-300 text-xs">
								<Flame className="h-3.5 w-3.5 text-orange-400" /> Calories
							</div>
							<p className="mt-1 text-zinc-100 font-semibold">
								{estimatedCalories} kcal
							</p>
						</div>
						<div className="rounded-xl bg-zinc-800/80 p-3">
							<div className="flex items-center gap-2 text-zinc-300 text-xs">
								<Zap className="h-3.5 w-3.5 text-orange-400" /> Weekly
							</div>
							<p className="mt-1 text-zinc-100 font-semibold">
								{weeklyCompletionPercent}%
							</p>
						</div>
					</div>

					<button className="mt-5 w-full sm:w-auto rounded-2xl bg-orange-500 px-6 py-3 text-sm font-semibold text-white transition hover:bg-orange-400 active:scale-[0.98]">
						Start Workout
					</button>

					<div className="mt-5">
						<div className="mb-2 flex items-center justify-between text-xs text-zinc-400">
							<span>Weekly completion</span>
							<span>{weeklyCompletionPercent}%</span>
						</div>
						<AnimatedProgressBar value={weeklyCompletionPercent} />
					</div>
				</>
			)}
		</BaseCard>
	)
}

export default HeroWorkoutCard
